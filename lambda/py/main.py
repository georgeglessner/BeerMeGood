...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        handler_input.response_builder.speak(speech_text).set_card(SimpleCard('Beer Me'
...
class BeerMeIntent(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        f = urllib.request.urlopen('http://beermapping.com/webservice/loccity/{}/{}&s=json'.format(api_key,
                                   city))
            ...
            for i in range(0, totalCount):
                if parsed_json[i]['status'] in ['Brewpub', 'Brewery']:
                    breweries.append({
                        ...
                        'street': parsed_json[i]['street'],
                        'locID': parsed_json[i]['id'],
                        'overall': parsed_json[i]['overall'],
                        })

        randInt = randint(0, len(breweries))

        speech_text = '{} located at {}'.format(breweries[randInt].get('name'),
                breweries[randInt].get('street'))
...
class BeerMeIntentState(AbstractRequestHandler):
    ...
    def can_handle(self, handler_input):
        ...
        return is_intent_name('BeerMeIntentState')(handler_input)
    ...
    def handle(self, handler_input):
        ...
        f = urllib.request.urlopen('http://beermapping.com/webservice/loccity/{}/{},{}&s=json'.format(api_key,
                                   city, state))
        print("*** URL *** ", 'http://beermapping.com/webservice/loccity/{}/{},{}&s=json'.format(api_key,
                                   city, state))
        json_string = f.read()
        parsed_json = json.loads(json_string)
        print(parsed_json)
...
class TopBreweryIntent(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech_text = '{} located at {}'.format(max(seq)[1],
                max(seq)[2])
...
class TopBreweryIntentState(AbstractRequestHandler):
    ...
    def can_handle(self, handler_input):
        ...
        return is_intent_name('TopBreweryIntentState')(handler_input)
...
class HelpIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        handler_input.response_builder.speak(speech_text).ask(speech_text).set_card(SimpleCard('Beer Me'
                , speech_text))
        return handler_input.response_builder.response
...
class FallbackIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech_text = "The Beer Me skill can't help you with that.  You can ask for a brewery in a city!"
...
sb.add_request_handler(TopBreweryIntentState())
